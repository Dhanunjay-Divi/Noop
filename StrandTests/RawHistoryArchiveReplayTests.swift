...
final class RawHistoryArchiveReplayTests: XCTestCase {
    ...
    #if os(iOS)
    /// Locked/background BLE must be able to archive rejects before acknowledging the strap trim.
    /// Simulator runtimes do not always report protection attributes, so that environment is skipped.
