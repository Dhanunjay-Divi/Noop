...
final class SleepNapDebtTests: XCTestCase {
    ...
    func testStageLessNapDoesNotGuessFromItsInBedWindow() {
        let nightStart = midnight - 60 * 60
        let napStart = midnight + 14 * 60 * 60
        let night = session(
            start: nightStart, durationMin: 416,
            stages: #"{"awake":24,"light":214,"deep":82,"rem":96}"#)
        let stageLessNap = session(start: napStart, durationMin: 50, stages: nil)
