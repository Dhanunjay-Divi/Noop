...
object OuraDecoders {
    ...
    /** Decode 0x5D as unsigned `(average HR bpm, average RMSSD ms)` five-minute pairs. */
    fun decodeHRV(rec: OuraRecord): List<OuraHRV>? {
        val b = rec.payload
        if (b.size < 2 || b.size % 2 != 0) return null
        val pairCount = b.size / 2
    ...
    fun decodeSpO2DC(rec: OuraRecord): List<OuraSpO2>? {
        ...
        if (hasBase) {
            out.add(
                OuraSpO2(ringTimestamp = rec.ringTimestamp, value = acc, unit = "dc_raw", index = 0),
            )
        }
        while (i < b.size) {
            val v = i8(b[i])
