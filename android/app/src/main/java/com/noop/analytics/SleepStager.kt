...
object SleepStager {
    ...
    internal fun respRateFromRR(rr: List<RrInterval>, start: Long, end: Long): Double {
        ...
        val inBedRows = rr.asSequence()
            ...
            .toList()
        // RSA requires real beat-to-beat intervals, not a banked record's coarse-time decomposition.
        // Reuse the HRV trust boundary so SDNN and respiration judge the same R-R evidence identically.
        if (inBedRows.size >= 30) {
            val fraction = HrvAnalyzer.beatAccurateFraction(
                inBedRows.map { it.ts },
                inBedRows.map { it.rrMs.toDouble() },
            )
    ...
    /** Explicit respiration evidence; absence is not represented as measured regularity. */
