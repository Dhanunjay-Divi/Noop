...
internal fun stepsForDay(apple: List<AppleDaily>, healthConnect: List<AppleDaily>, dayKey: String): Int? =
    (apple + healthConnect)
        ...
        .mapNotNull { it.steps?.takeIf { value -> value >= 0 } }
        .maxOrNull()

/**
 * Single-day primary Steps arbitration shared by cards and tests.
 *
 * Wrist motion and heart rate can describe movement or effort but cannot prove footfalls. Both local
 * estimates remain outside the primary Steps surface.
 */
...
/**
 * Truthful Today copy for the selected step winner. Only an imported Health Connect / Apple Health
 * pedometer count can populate primary Steps.
 */
@Suppress("UNUSED_PARAMETER")
...
/** Compact Key-Metrics label. Unvalidated local motion never changes the primary metric name. */
@Suppress("UNUSED_PARAMETER")
