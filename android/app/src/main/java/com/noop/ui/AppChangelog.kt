    ...
    val releases: List<Release> = listOf(
        ...
        Release(
            ...
            items = listOf(
                ...
                "**Your nutrition and Strength Trainer data leave with you.** Export data now creates one open, readable ZIP that keeps the existing WHOOP-compatible CSVs and adds versioned NOOP JSON for meals, exercises, routines, sessions, sets, RPE, rest, notes, and archive state. It re-imports on iPhone, Mac, or Android without letting an older export overwrite a newer local edit.",
                "**Safety messages and check-ins fail visibly.** A one-shot location expires after five minutes instead of silently sharing an old fix, disabled reminder delivery is shown with a settings or repair path, and tapping the reminder returns directly to Safety. NOOP still does not monitor, message contacts, or dispatch emergency help.",
                "**Safety now speaks the app's language.** The complete Safety Center, user-reviewed draft, local reminder, and Android reminder channel are localized across all nine maintained languages. VoiceOver and TalkBack get full labels for compact choices and clearer single-action controls; NOOP still never monitors or dispatches emergency services.",
                "**Nutrition totals no longer double-count imports.** When an imported daily summary and manual meals share a day, imported values take priority per nutrient and manual meals fill only missing fields. Recent manual meals can now be logged again in one tap, and localized decimal/grouping separators round-trip safely.",
            ),
        ),
        Release(
            version = "9.1.2",
        ...
        Release(
            ...
            date = "June 2026",
            items = listOf(
                "**New: Backup & Restore.** You can now preserve this device's NOOP database (including history, scores, sleep, workouts, nutrition, and strength data) plus durable profile, display, dashboard, Sleep Planner, and reminder settings in a folder you choose, on demand or on a daily schedule, and restore it later. Credentials, hardware bindings, permissions, and volatile delivery state are excluded. It's off by default, runs entirely on your device, validates the backup, and keeps a safety snapshot first so a failed restore cannot wipe your data. Find it in Settings.",
        ...
        Release(
            ...
            items = listOf(
                ...
                "**New: Automatic workout detection.** NOOP spots a likely workout from your heart rate and motion. Detection is approval-first: Ask mode never writes a workout until you confirm it.",
        ...
        Release(
            ...
            items = listOf(
                ...
                "Polish from a full code review: your Vitality breakdown now reconciles with its Wellness Age comparison; profile-age bounds match iPhone/Mac; and live workout detection covers the whole calendar day. Thanks @rad182, @cbarrado, @j0b-dev.",
            ),
        ),
        Release(
            version = "4.0.0",
        ...
        Release(
            ...
            items = listOf(
                ...
                "New (iPhone): an AltStore / SideStore source for one-tap updates on sideloaded installs. The current stable channel is https://github.com/Dhanunjay-Divi/Noop/releases/download/altstore-source/altstore-source.json and works only while its manifest and IPA assets are public. Reimplemented from @RazvanRex (#178).",
            ),
        ),
        Release(
            version = "2.6.2",
        ...
        Release(
            ...
            date = "June 2026",
            items = listOf(
                "New (Mac, iOS and Android, experimental): a WHOOP 5.0/MG \"deep data\" unlock under Settings → Experimental. 5/MG straps give a fresh third-party app only live heart rate; the official app switches on the deeper streams by writing a set of feature flags. NOOP can now send that exact, documented sequence to your strap (opt-in, one button, only when worn + bonded). It writes to the strap but is reversible - it just changes which data the strap emits - and it is the same thing the official app does. Experimental: it may do nothing on your firmware yet. If you have a 5/MG, turning it on and sharing your strap log is exactly what we need to finish 5.0/MG support. iPhone/Android only (a Mac cannot write to a 5/MG). Built from published interoperability facts and NOOP's own verification. (#174)",
            ),
        ),
        Release(
            version = "2.4.0",
