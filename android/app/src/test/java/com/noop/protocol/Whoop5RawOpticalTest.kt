 ...
 * `Whoop5RawOpticalTests` runs the same assertions through its own decoder. Both records are deterministic
 * and fully synthetic, so neither platform can drift without failing. The fixture contains no device or
 * participant data.
 */
class Whoop5RawOpticalTest {
    ...
    /**
     * Sign extension. The synthetic containers are already sign-extended, so the failure this guards is
     * a decoder that reads them unsigned or re-extends from bit 19: either turns `c0 1d fe ff` into a
     * large positive number instead of -123,456.
     */
    @Test
    ...
    fun saturationRailIsExactly2Pow19Minus1() {
        ...
        val record = (0 until records.length())
            ...
            .first { it.getString("name") == "synthetic_rails_and_u32_header" }
        val decoded = requireNotNull(Whoop5RawOptical.decode(hexToBytes(record.getString("hex"))))
    ...
    fun fullCorpusDecodesWithPublishedInvariants() {
        ...
        Assume.assumeTrue(
            "set WHOOP_R20_CORPUS to a local R20 JSONL to run the opt-in corpus check",
    ...
    /**
     * A minimal, structurally valid, checksum-sealed v20 frame. Sealing is required because the decoder
     * CRC-gates every frame, including synthetic test input.
     */
