...
class AnalysisInputGateContractTest {
    ...
    fun launchResumeLoopClaimsBoundedSourcesAndDoesNotScanHistory() {
        ...
        assertTrue(loop.contains("force = forceAnalysis"))
        assertTrue(loop.contains("if (!analysisPlan.shouldAnalyze)"))
        assertTrue(loop.contains("exclusionResult?.madeProgress == true -> \"excluded\""))
        assertTrue(loop.contains("else -> \"deferred\""))
        assertTrue(loop.contains("\"late_evening\""))
        assertTrue(loop.contains("\"timezone_provenance\""))
        assertTrue(loop.contains("\"truncated_history\""))
        assertTrue(loop.contains("\"travel_boundary\""))
        assertTrue(loop.contains("\"timezone_terminal_unknown\""))
        assertTrue(loop.contains("\"scan_days\" to \"0\""))
    ...
    fun postBackfillPathUsesGenerationGateWithoutLegacyWatermark() {
        ...
        val end = source.indexOf("\n    private var backfilling", start)
        assertTrue(start >= 0 && end > start)
        val pass = source.substring(start, end)

        assertTrue(pass.contains("repository.claimAnalysisInput("))
        assertTrue(pass.contains("exclusionResult?.madeProgress == true -> \"excluded\""))
        assertTrue(pass.contains("else -> \"deferred\""))
        assertTrue(pass.contains("\"late_evening\""))
        assertTrue(pass.contains("\"timezone_provenance\""))
        assertTrue(pass.contains("\"truncated_history\""))
        assertTrue(pass.contains("\"travel_boundary\""))
        assertTrue(pass.contains("\"timezone_terminal_unknown\""))
        assertTrue(pass.contains("\"scan_days\" to \"0\""))
    ...
    fun archiveLifecycleClearsOwnershipAndKicksOneRecompute() {
        ...
        assertTrue(viewModelPath.contains("ArchivedDeviceResult"))
        assertTrue(viewModelPath.contains("deviceRegistry.all()"))
        assertTrue(viewModelPath.contains("DeviceStatus.active.name"))
        assertTrue(viewModelPath.contains("noopApp.activeDeviceIdFlow.value"))
        assertTrue(viewModelPath.contains("noopApp.noteActiveDeviceId(selectedId)"))
        assertTrue(viewModelPath.contains("_selectedDeviceId.value = selectedId"))
