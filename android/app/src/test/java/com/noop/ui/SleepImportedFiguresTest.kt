...
class SleepImportedFiguresTest {
    ...
    @Test
    fun stageLessNapDoesNotGuessFromItsInBedWindow() {
        val zone = ZoneId.systemDefault()
        val midnight = LocalDate.of(2026, 6, 2).atStartOfDay(zone).toEpochSecond()
        val night = SleepSession(
            deviceId = "my-whoop", startTs = midnight - 60 * 60, endTs = midnight + 356 * 60,
            stagesJSON = """{"awake":24,"light":214,"deep":82,"rem":96}""",
        )
        val napStart = midnight + 14 * 60 * 60
        val stageLessNap = SleepSession(
            deviceId = "my-whoop", startTs = napStart, endTs = napStart + 50 * 60,
            stagesJSON = null,
        )

        val credit = napSleepMinutesByDay(listOf(night, stageLessNap))
        assertEquals(0.0, credit["2026-06-02"]!!, 1e-9)
    }
