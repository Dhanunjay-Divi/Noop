...
final class SchemaOracleTests: XCTestCase {
    ...
    private func iosExpectedIndices(_ table: OracleTable) -> [OracleIndex] {
        table.indices.filter { $0.iosAbsent != true }
    }
    ...
    func testGrdbSchemaMatchesOracle() async throws {
        ...
        for (name, table) in oracle.tables.sorted(by: { $0.key < $1.key }) {
            ...
            let expectedIndices = iosExpectedIndices(table)
            if actual.indices.map({ $0.name }) != expectedIndices.map({ $0.name }) {
                problems.append("\(name): index names — oracle \(expectedIndices.map { $0.name }), "
                                + "grdb \(actual.indices.map { $0.name })")
            }
            for want in expectedIndices {
    ...
    func testDivergenceOverridesAreWellFormed() throws {
        ...
        for (name, table) in oracle.tables {
            ...
            for index in table.indices {
                if index.iosAbsent == true || index.androidAbsent == true {
                    XCTAssertNotNil(index.divergence,
                                    "\(name).\(index.name): platform-absent index with no reason key")
                }
                XCTAssertFalse(index.iosAbsent == true && index.androidAbsent == true,
                               "\(name).\(index.name): index absent on both platforms — delete it")
            }
