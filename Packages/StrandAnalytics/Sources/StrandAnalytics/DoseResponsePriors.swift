...
public struct DoseResponsePrior: Equatable, Sendable {
    ...
    /// The product-facing outcome label this prior is expressed in (e.g. "Recovery", "HRV").
    public let outcome: String
    /// Typical signed effect per ONE extra unit of dose (e.g. −X Recovery points per drink).
...
public enum DoseResponsePriors {
    ...
    public static func prior(for behavior: DosedBehavior, outcome: String) -> DoseResponsePrior? {
        // `Charge` was the pre-NOOP-terminology label. Accept it as a lookup alias so persisted
        // experiments and older callers still resolve, while every returned/rendered prior carries
        // the canonical product label `Recovery`.
        let canonical = outcome == "Charge" ? "Recovery" : outcome
        return table.first { $0.behavior == behavior && $0.outcome == canonical }
    }
