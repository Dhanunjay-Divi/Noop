...
/// `android/app/src/test/resources/r20_optical_oracle.json`. Both platforms decode the same deterministic,
/// fully synthetic records and assert the same explicit expectations, so a one-sided offset edit fails on
/// the platform that moved, not silently on both. The fixture contains no device or participant data.
final class Whoop5RawOpticalTests: XCTestCase {
    ...
    /// The synthetic block pattern has three sampling blocks and two sentinel-filled inactive blocks.
    /// `sample_count == 0` must yield no readings even when the physical slots are nonzero.
    func testBlockPatternIsThreeActiveTwoDark() throws {
        ...
        for record in oracle.records {
            ...
            // Block 3 is the fixture's synthetic dark control: sampling, but with both drives at zero.
    ...
    /// Sign extension. The synthetic containers are already sign-extended, so the failure this guards
    /// is a decoder that reads them unsigned or re-extends from bit 19: either turns `c0 1d fe ff` into
    /// a large positive number instead of -123,456.
    func testNegativeReadingsSignExtend() throws {
        ...
        // The exact little-endian container from the fixture's first synthetic record.
        let first = try XCTUnwrap(Whoop5RawOptical.decode(hexToBytes(oracle.records[0].hex)))
        XCTAssertEqual(first.blocks[0].readingsB.first, -123_456,
                       "bytes c0 1d fe ff are -123,456; a large positive value means the sign was dropped")
    }

    /// The positive rail: exactly 2^19-1, never exceeded anywhere in the synthetic oracle. A decoder
    /// that mis-sizes the container would land somewhere else entirely.
    func testSaturationRailIsExactly2Pow19Minus1() throws {
        let oracle = try loadOracle()
        let record = try XCTUnwrap(oracle.records.first { $0.name == "synthetic_rails_and_u32_header" },
    ...
    /// and anything built on it rejects a correctly sealed R20 envelope.
    ...
    /// Optionally runs the decoder over a locally supplied R20 JSONL corpus and asserts the published
    /// invariants on every record. The committed golden vectors remain fully synthetic; this opt-in path
    /// is skipped unless a developer explicitly supplies a local corpus.
    ///
    /// Opt-in; external corpora must not be committed:
    ///
    ///     WHOOP_R20_CORPUS=/path/to/buffers2140.jsonl swift test --filter testFullCorpus
    ///
    /// Expected input is one JSON object per line with `hex`, `size`, and optional `strap_ts` fields.
    func testFullCorpusDecodesWithPublishedInvariants() throws {
        guard let path = ProcessInfo.processInfo.environment["WHOOP_R20_CORPUS"] else {
            throw XCTSkip("set WHOOP_R20_CORPUS to a local R20 JSONL to run the opt-in corpus check")
        }
    ...
    /// A minimal, structurally valid, checksum-sealed v20 frame. Sealing is required because the
    /// decoder CRC-gates every frame, including synthetic test input.
