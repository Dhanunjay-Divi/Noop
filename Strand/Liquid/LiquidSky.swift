...
/// The shared obsidian atmosphere for any `ScreenScaffold.topBackground`.
///
/// Today, Trends, Sleep and More now share the same sculpted black canvas. This
/// deliberately replaces the previous purple/blue sky on primary navigation:
/// time-of-day colour was overpowering the data and made each page feel themed
/// instead of forming one product. The legacy sky renderer remains below for
/// compatibility with promo/demo surfaces that explicitly request it.
///
...
/// - "Dimensional background" OFF renders nothing, leaving the scaffold's plain `surfaceBase` canvas
///   (the same visual as passing no topBackground at all).
/// - "Background behind cards" ON fills the scaffold's whole backdrop, so the
///   Card-transparency setting reveals the material under every card.
...
struct LiquidScaffoldSky: View {
    ...
    var body: some View {
        if showDayCycleBackground {
            if skyBehindCards {
                ObsidianFlowBackground(compact: false, intensity: 0.90)
                ...
                ZStack {
                    ...
                    ObsidianFlowBackground(compact: true, intensity: 0.84)
                }
