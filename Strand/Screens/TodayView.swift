...
private struct ActiveWorkoutIndicatorCard: View {
    ...
    private var sportLabel: some View {
        HStack(spacing: NoopMetrics.space3) {
            // A workout is the one dashboard state where continuous motion communicates real
            // activity. The runner stops the instant this card leaves the tree; Reduce Motion
            // receives the same dimensional illustration as a static frame.
            SemanticBodyIllustration(
                .workout(systemImage: sportSymbol(model.sport)),
                size: 46,
...
struct TodayView: View {
    ...
    // Query gate mirrored by a zero-size leaf. It blocks through short inter-session gaps in a deep
    // offload, so cold mounts and refreshSeq bumps cannot start broad reads against the write burst.
    ...
    /// Resolve the Classic Today respiratory card's value and tap-through source together. A current
    /// WHOOP reading wins, then the explicit prior-WHOOP carry used while today's overnight vital is not
    /// available, then Apple Health. Keeping the value and source in one result prevents a carried WHOOP
    /// number from opening an unrelated Apple Health detail. No available value keeps the existing Apple
    /// destination, where import/setup guidance is actionable.
    static func respiratoryDashboardReading(
        todayWhoop: Double?,
        priorWhoop: Double?,
        apple: Double?
    ) -> (value: Double?, source: String) {
        if let todayWhoop { return (todayWhoop, "my-whoop") }
        if let priorWhoop { return (priorWhoop, "my-whoop") }
        return (apple, "apple-health")
    }
    ...
    var body: some View {
        ScreenScaffold(title: scaffoldTitle, onRefresh: { await repo.refresh() },
                       ...
                       // Unlike the adaptive pearl/obsidian scaffold field, this photographic scene
                       // carries its own dark header scrim in both appearances.
                        ...
                        .overlay(alignment: .topTrailing) {
                            todayCardDismissButton {
                                dismissTodayCard(
                                    ...
                                    message: String(localized: "Recovery, Effort and Sleep Score build over your next few nights of wear.")
                                )
                            }
                        }
        ...
        // The query gate stays raised across short continuation gaps and is part of the task identity. Its
        // stable quiet edge therefore runs exactly one catch-up pass even when raw rows changed without a
        // daily-cache `refreshSeq` bump.
    ...
    private func dashboardCardRow(_ card: DashboardCard) -> some View {
        ...
        case .fitnessAge:
            pinnedCardRow(icon: card.icon, tint: tint, title: card.title, subtitle: card.subtitle,
                          value: dashboardValue(card),
                          route: .metricSourced(key: "fitness_age", source: "my-whoop"))
        case .vitality:
            pinnedCardRow(icon: card.icon, tint: tint, title: card.title, subtitle: card.subtitle,
                          value: dashboardValue(card),
                          route: .metricSourced(key: "vitality", source: "my-whoop"))
        case .steps:
            ...
            pinnedCardRow(icon: card.icon, tint: tint, title: card.title, subtitle: stepsSourceCaption,
                          value: dashboardValue(card), route: .metricSourced(
                            key: metric?.key ?? "steps", source: metric?.source ?? "my-whoop"))
        case .calories:
            pinnedEnergyCardRow(card)
        case .hrv:
            pinnedCardRow(icon: card.icon, tint: tint, title: card.title, subtitle: card.subtitle,
                          value: dashboardValue(card),
                          route: .metricSourced(key: "hrv", source: "my-whoop"))
        case .restingHr:
            pinnedCardRow(icon: card.icon, tint: tint, title: card.title, subtitle: card.subtitle,
                          value: dashboardValue(card),
                          route: .metricSourced(key: "rhr", source: "my-whoop"))
        case .respiratory:
            ...
            pinnedCardRow(icon: card.icon, tint: tint, title: card.title, subtitle: card.subtitle,
                          value: dashboardValue(card),
                          route: .metricSourced(key: "resp_rate", source: reading.source))
        case .bloodOxygen:
            pinnedCardRow(icon: card.icon, tint: tint, title: card.title, subtitle: card.subtitle,
                          value: dashboardValue(card),
                          route: .metricSourced(key: "spo2", source: "my-whoop"))
        case .skinTemp:
            pinnedCardRow(icon: card.icon, tint: tint, title: card.title, subtitle: card.subtitle,
                          value: dashboardValue(card),
                          route: .metricSourced(key: "skin_temp", source: "my-whoop"))
        case .sleep:
    ...
    private func dashboardValue(_ card: DashboardCard) -> String {
        ...
        func withUnit(_ s: String) -> String {
            guard s != StrandFormat.missing else { return StrandFormat.missing }
        ...
        case .hrv:
            ...
            #endif
            return withUnit(d?.avgHrv.map { "\(Int($0.rounded()))" } ?? StrandFormat.missing)
        case .restingHr:
            #if DEBUG
            if let f = DemoDayHarness.active { return withUnit("\(f.rhrBpm)") }
            #endif
            return withUnit(d?.restingHr.map { "\($0)" } ?? StrandFormat.missing)
        case .respiratory:
            let reading = Self.respiratoryDashboardReading(
                todayWhoop: d?.respRateBpm,
                priorWhoop: lastVitalsDay?.respRateBpm,
                apple: sparks["resp_rate"]?.last)
            return withUnit(reading.value.map { String(format: "%.1f", $0) } ?? StrandFormat.missing)
        case .bloodOxygen:
            ...
            return (d?.spo2Pct ?? lastVitalsDay?.spo2Pct ?? lastSpo2Day?.spo2Pct)
                .map { String(format: "%.0f%%", $0) } ?? StrandFormat.missing
        case .skinTemp:
            // The overloaded field is either a signed local deviation or an absolute WHOOP import.
            // Only deviations get a +/- prefix; "+34°" would falsely describe an absolute reading as
            // a 34-degree rise from baseline.
            return (d?.skinTempDevC ?? lastVitalsDay?.skinTempDevC ?? lastSkinTempDay?.skinTempDevC)
                .map {
                    VitalBands.isAbsoluteSkinTemp($0)
                        ? String(format: "%.1f°", $0)
                        : String(format: "%+.1f°", $0)
        ...
        case .sleep:
            return sleepValue(d)
        case .steps:
            // A same-day measured phone count outranks a classified WHOOP 5/MG @57 counter estimate.
            // Gravity-only calibration is not gait evidence and is deliberately withheld here.
        ...
        case .calories:
            return selectedEnergyBreakdown.headlineKcal.map { "\(intString($0)) kcal" } ?? StrandFormat.missing
        case .stress:
            #if DEBUG
        ...
        case .fitnessAge:
            ...
            return value.map(FitnessAgePresentation.value) ?? StrandFormat.missing
        case .vitality:
            let value = ageMetricsLoadedProfileState == profile.ageMetricStateToken ? vitalityToday : nil
            return value.map { "\(Int($0.rounded()))" } ?? StrandFormat.missing
        case .hydration:
            guard let goal = hydrationGoalML else { return StrandFormat.missing }
    ...
    /// only for a complete Apple split; partial Apple days lead with their real component, while the
    /// strap-only path is explicitly a combined estimate with no fabricated Active/Resting breakdown.
    private func pinnedEnergyCardRow(_ card: DashboardCard) -> some View {
        let breakdown = selectedEnergyBreakdown
        let metric = selectedEnergyMetric
        return NavigationLink(value: TabRoute.metricSourced(
            key: metric?.key ?? "energy_kcal", source: metric?.source ?? "my-whoop")) {
            VStack(alignment: .leading, spacing: 10) {
                HStack(spacing: 10) {
                    RoundedRectangle(cornerRadius: 9, style: .continuous)
                        .fill(StrandPalette.metricAmber.opacity(0.14))
                        .frame(width: 34, height: 34)
                        .overlay(Image(systemName: card.icon).font(.system(size: 15, weight: .semibold))
                            .foregroundStyle(StrandPalette.metricAmber))
                    VStack(alignment: .leading, spacing: 2) {
                        HStack(spacing: 5) {
                            Text(card.title.uppercased()).font(StrandFont.overline)
                                .tracking(StrandFont.overlineTracking)
                                .foregroundStyle(StrandPalette.textPrimary)
                            Image(systemName: "info.circle")
                ...
                HStack(alignment: .bottom, spacing: 12) {
                    VStack(alignment: .leading, spacing: 1) {
                        Text(energyKcalText(breakdown.headlineKcal))
                            .font(StrandFont.number(24))
                            .foregroundStyle(StrandPalette.textPrimary)
                            .lineLimit(1).minimumScaleFactor(0.65)
                        Text(energyHeadlineLabel(breakdown).uppercased()).strandOverline()
                    }
                    Spacer(minLength: 4)
                    energyDashboardMini("ACTIVE", breakdown.activeKcal)
                    energyDashboardMini("RESTING", breakdown.restingKcal)
                }
            }
            .padding(.horizontal, 13).padding(.vertical, 12)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(FrostedCardSurface(tint: StrandPalette.metricAmber,
                                           cornerRadius: NoopMetrics.cardRadius, washStrength: 0.62))
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
    ...
    private func energyDashboardMini(_ label: String, _ value: Double?) -> some View {
        VStack(alignment: .trailing, spacing: 1) {
            Text(value.map { intString($0) } ?? StrandFormat.missing)
        ...
        NoopCard(tint: StrandPalette.chargeColor) {
            VStack(spacing: 0) {
                ...
                metricRow(icon: "waveform.path.ecg", label: "HRV",
                          value: demoHrv ?? (hrv.map { "\(Int($0.rounded()))" } ?? StrandFormat.missing), unit: "ms",
                          tint: StrandPalette.metricCyan)
                Divider().overlay(StrandPalette.hairline)
                metricRow(icon: "heart.fill", label: "Resting HR",
                          value: demoRhr ?? (rhr.map { "\($0)" } ?? StrandFormat.missing), unit: "bpm",
                ...
                metricRow(icon: "lungs.fill", label: "Respiratory",
                          ...
                          value: resp.map { String(format: "%.1f", $0) }
                              ?? (vd == nil ? latestString("resp_rate", decimals: 1) : StrandFormat.missing),
                          unit: "rpm",
    ...
    private func carriedVital(
        ...
        if let honest = Self.emptyVitalCaption(unit: unit, isToday: selectedDayOffset == 0) {
            return (StrandFormat.missing, honest)
        }
        return (StrandFormat.missing, unit)
    }
    ...
    private func keyMetricTileContent(_ metric: KeyMetric) -> some View {
        ...
        case .charge:
            ...
            StatTile(
                ...
                value: d?.recovery.map { "\(Int($0.rounded()))%" }
                    ...
                    ?? carried.map { "\(Int($0.value.rounded()))%" } ?? StrandFormat.missing,
        ...
        case .effort:
            ...
            StatTile(
                label: "Effort",
                value: effort.map { UnitFormatter.effortDisplay($0, scale: effortScale) } ?? StrandFormat.missing,
                systemImage: systemImage,
        ...
        case .rest:
            ...
            StatTile(
                label: "Sleep Score",
                value: restScore.map { "\(Int($0.rounded()))%" } ?? StrandFormat.missing,
                systemImage: systemImage,
        ...
        case .hrv:
            ...
            StatTile(
                ...
                systemImage: systemImage,
                caption: hrv.caption,
                accent: hrv.value == StrandFormat.missing
                    ? StrandPalette.textPrimary
        ...
        case .restingHr:
            ...
            StatTile(
                ...
                systemImage: systemImage,
                caption: rhr.caption,
                accent: rhr.value == StrandFormat.missing
                    ? StrandPalette.textPrimary
        ...
        case .bloodOxygen:
            ...
            StatTile(
                ...
                accent: spo2.value == StrandFormat.missing
                    ? StrandPalette.textPrimary
        ...
        case .steps:
            ...
            // classified motion-derived estimate. A gravity-only calibrated estimate is not gait proof
            // and no longer fills the primary Steps tile.
            ...
            StatTile(
                ...
                accessory: {
                    if importedSteps == nil, strapMotionSteps != nil, let cls = stepActivityClassToday {
    ...
    private func energyKeyMetricTile(systemImage: String) -> some View {
        let breakdown = selectedEnergyBreakdown
        return NoopCard(padding: 14, tint: StrandPalette.metricAmber) {
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 4) {
                    Text(breakdown.headlineKcal.map { intString($0) } ?? StrandFormat.missing)
                        .font(StrandFont.number(24))
                        .foregroundStyle(breakdown.headlineKcal == nil
                            ? StrandPalette.textTertiary : StrandPalette.metricAmber)
                ...
                HStack(spacing: 8) {
                    ...
                    Divider().overlay(StrandPalette.hairline).frame(height: 22)
                    energyKeyMini(String(localized: "Resting"), breakdown.restingKcal)
                }
            }
        }
        .accessibilityElement(children: .combine)
        .accessibilityLabel(
            String(localized: "Calories") + ", "
                + energyKcalText(breakdown.headlineKcal) + ", "
                + energyStateText(breakdown)
        )
    }
    ...
    /// Honest band-sync outcome, the live-observing subview (BandSyncRow) renders it. Kept as a
    ...
    private var bandSyncRow: some View { BandSyncRow() }
    ...
    private func loadAll() async {
        ...
        // long-lived `repo` (not @State), so it survives the re-mount that resets view-local state.
    ...
    private func loadHistoryWide() async -> Bool {
        ...
        async let stepsAppleSpark    = sparkPoints("steps", source: "apple-health", window: 14)
    ...
    private func sparkValues(_ key: String, source: String, window: Int) async -> [Double] {
        let points = await sparkPoints(key, source: source, window: window)
        return points.map(\.value)
    }

    /// Day-preserving sibling used when multiple step sources must arbitrate per calendar day.
    private func sparkPoints(_ key: String, source: String, window: Int) async -> [(day: String, value: Double)] {
        ...
        return trailingWindow(all, days: window)
    }
    ...
    private func latestString(_ key: String, decimals: Int, unit: String = "") -> String {
        guard let last = sparks[key]?.last else { return StrandFormat.missing }
    ...
    private func weightTile(_ appleWeightKg: Double?) -> (value: String, caption: String) {
        ...
        return (StrandFormat.missing, selectedDayOffset == 0
    ...
    private func synthesisDetail(_ d: DailyMetric?) -> String {
        ...
        switch RecoveryBandPresentation.level(for: rec) {
        case .low:
            switch sleptWell {
    ...
    private func sleepValue(_ d: DailyMetric?) -> String {
        guard let m = d?.totalSleepMin else { return StrandFormat.missing }
    ...
    static let needsStrapCaption = String(localized: "Needs wearable data")
    ...
    /// Selected-day source-isolated energy. `activeKcalEst` is a legacy-named combined HR estimate,
    /// never an Active Energy component. The resolver gives Apple Health precedence as a whole source;
    /// a missing Apple component stays missing instead of being filled from the strap.
    private var selectedEnergyBreakdown: DailyEnergyBreakdown {
        let apple = appleDays.last(where: { $0.day == selectedDayKey })
        return DailyEnergyBreakdown.resolve(
            appleActiveKcal: apple?.activeKcal,
            appleRestingKcal: apple?.basalKcal,
            wearableCombinedKcal: displayDay?.activeKcalEst
        )
    }

    private var selectedEnergyMetric: MetricDescriptor? {
    ...
    private func energyKcalText(_ value: Double?) -> String {
        value.map { "\(intString($0)) kcal" } ?? StrandFormat.missing
    }
...
private struct RecordingStatusLight: View {
    ...
    private var syncingAccessibilityLabel: String {
        ...
        return n > 0
            ? String(localized: "Syncing Noop Band history, chunk \(n)")
            : String(localized: "Syncing Noop Band history")
    }
}
...
/// Honest band-sync outcome row for the Data Sources card (ports the Android Live line, ed6a31d): the
...
private struct BandSyncRow: View {
    @EnvironmentObject private var live: LiveState
    var body: some View {
        if !live.backfilling {
    ...
    var body: some View {
        if live.connected, let pct = live.batteryPct {
            ...
            HStack(spacing: 10) {
                ...
                .accessibilityElement(children: .ignore)
                .accessibilityLabel("Noop Band battery \(Int(pct.rounded())) percent\(live.charging == true ? ", charging" : "")\(estimateText.map { ", \($0)" } ?? "")")
            }
        }
    }
}
...
enum MetricTileState: Equatable {
    ...
    var title: LocalizedStringKey? {
        ...
        case .needsStrap:                   return "Needs wearable data"
        }
    }
...
enum RecordingState: Equatable {
    ...
    /// Link is up, but no live HR or usable history has arrived yet.
    ...
    static func resolve(connected: Bool,
        ...
        if connected && (lastSyncedAt == nil || sustainedEmptyOffload) { return .connectedNoData }
