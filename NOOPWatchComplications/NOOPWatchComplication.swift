...
//
// The headline feature of M3: Recovery on the wrist. The iPhone is the brain
// (M1 computes Recovery / Effort / Sleep with confidence + provenance); this complication ONLY
...
struct ChargeProvider: TimelineProvider {
    /// A friendly stand-in for the gallery / first paint. Shows a real-looking Recovery score so the
...
struct NOOPChargeView: View {
    ...
    // MARK: Recovery tint
    //
    // Tinted to the Recovery colour world only when we have a real number. A calibrating or missing
    // Recovery stays neutral so the empty ring never borrows a "good"/"bad" colour it did not earn.
    ...
    // MARK: accessoryCircular — a ring + the Recovery number
    //
    ...
    // and tint it to the Recovery colour where we have a real value. A small "cal" marker replaces the
    // number when Recovery is calibrating.
    ...
    //
    // The richest family: a small NOOP header line plus the Recovery / Effort / Sleep triplet, each a
    ...
    // MARK: Effort / Sleep tints (rectangular only)
