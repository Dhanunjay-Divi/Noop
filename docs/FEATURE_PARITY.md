...
| Safety Network / personal check-in | **Available, user-confirmed and non-emergency** | Onboarding and Safety invite two to five trusted contacts and keep a private reminder active until at least two have accepted. A user-confirmed page creates one durable incident, sends SMS first, falls back to voice while unacknowledged, records delivery/response state, and stops pending escalation after acknowledgement, resolution, cancellation, or expiry. Idempotency, leases, bounded retries, provider receipts, signed response links, and DTMF responses prevent a single transient failure from becoming silent loss. NOOP also retains the optional user-reviewed share-sheet message, five-minute one-shot location, and best-effort local check-in reminder. It never opens a page from wellness/anomaly scores, detects falls, or dispatches emergency services; carrier delivery and human response are not guaranteed. |
