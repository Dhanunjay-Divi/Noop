...
- NOOP-hosted, invitation-only per-friend daily-summary sharing with explicit
  field controls and no public directory, ranking, or contact upload.
...
| Safety Network / personal check-in | **Implemented, release evidence pending** | The first-release path is manual app-to-app paging of accepted NOOP Safety contacts. App confirmation creates one durable incident; repeated-band and automatic origins remain behind separate hardware and validation gates. APNs/FCM carries only an opaque incident reference, and authenticated recipients can respond or decline. Precise location is off by default and requires separate incident-scoped consent; only the newest fix is retained during the selected 8- or 12-hour window, never a route. SMS/voice remains disabled until carrier or DLT, legal, physical delivery, monitoring, failover, and staffed-operations gates pass. Wellness/anomaly values never page, NOOP never dispatches emergency services, and delivery or human response is not guaranteed. |
...
| WHOOP capability | Noop status | What Noop does, and what it does not claim |
|---|---|---|
| Cloud history and cross-device access | **Partial, opt-in** | Optional self-hosted sync sends its supported v1 decoded/derived subset only to a server the user configures. Separately consented NOOP+ implements bounded managed chunk upload and restore, but it is not authoritative for local collection or scoring. Its managed-history export/import resumes from private checkpoints and includes `day_ownership` as the only managed document in the current export scope. |
| Data export | **Available locally; managed production evidence incomplete** | Local backup/export remains available without NOOP+. Same-platform native backup and the supported portable Apple/Android subset have restore paths. NOOP+ can resume a snapshot-bound ZIP for selected managed chunks and `day_ownership`, completely validate it before mutation, and resume idempotent import. Live expiry, large-account, cancellation/auth-refresh, cross-tenant, low-storage, and physical process-death evidence remain open under `DAT-170`. |
...
| Teams, leaderboards, and social community | **Partial, private and NOOP-hosted** | Apple and Android Friends surfaces use a NOOP account that is separate from managed health-backup consent. Accepted friends share no health field or communication capability until the owner enables it for that relationship. The six bounded daily summary fields are directional, and message, photo, audio-call, and video-call permissions default off; those communication transports are not yet implemented. Friends is not currently end-to-end encrypted, so production enablement remains gated on the reviewed service, identity, abuse, deletion, and security controls. There is no public discovery, contact upload, follower model, ranking, team administration, or challenge product. |
...
| Legacy Friends compatibility consent | The retained self-hosted API uploads no biometric summary values until at least one friend is accepted; empty replacement maps may still clear an older Friends window. It then uploads only the union of fields currently enabled for accepted friends, from six allowlisted, range-checked `noop_computed` daily values. Member credentials cannot read raw, export, workout, journal, sleep-stage, route, device, or admin APIs. This protocol has no routed customer Friends screen. |
| Legacy Friends compatibility storage | The retained API writes through a dedicated logical `*-noop-friends` producer, separate from the full archive producer. A friend's feed begins on the friendship's acceptance date; earlier daily history is not projected. |
| Legacy Friends compatibility lifecycle | The lower-level compatibility clients retain bounded catch-up, deletion, and operator-trust controls for controlled migration. Current Apple, Android, and macOS Friends routes use the NOOP-hosted account service and never ask a customer to configure this server. |
