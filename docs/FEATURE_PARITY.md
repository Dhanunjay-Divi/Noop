...
| WHOOP capability | Noop status | What Noop does, and what it does not claim |
|---|---|---|
| Cloud history and cross-device access | **Partial, opt-in** | Optional self-hosted sync sends its supported v1 decoded/derived subset only to a server the user configures. Separately consented NOOP+ implements bounded managed chunk upload and restore, but it is not authoritative for local collection or scoring. Its managed-history exporter is non-resumable, has no importer, and includes `day_ownership` as the only managed document in the current export scope. |
| Data export | **Available locally; managed portability incomplete** | Local backup/export remains available without NOOP+. Same-platform native backup and the supported portable Apple/Android subset have restore paths. NOOP+ can create a snapshot-bound ZIP for selected managed chunks and `day_ownership`, but interrupted continuation, an archive importer, and complete live expiry/corruption/large-account evidence remain open under `DAT-170`. |
