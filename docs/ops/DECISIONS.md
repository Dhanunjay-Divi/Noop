...
| D-008 | Private development may continue while commercial or public distribution remains blocked by unresolved repository-rights status. Passing the inventory check is not the same as passing the distribution gate. | Active | `Tools/release-legal-gate.py`, `docs/provenance/rights-status.json` |
...
| D-011 | Standalone hosting, fork metadata, and Git history presentation do not establish source ownership. Required attribution stays until affected code is licensed, independently replaced, or removed with reviewed evidence. | Active | `docs/REPOSITORY_INDEPENDENCE.md`, `docs/provenance/rights-status.json`, `Tools/release-legal-gate.py` |
