...
| `swift test` in `Packages/NoopRemoteSync` | 97 passed | Apple transport, loopback restriction, retry, restore, export, social, and existing sync contracts | App-target compilation or simulator UI |
...
| Fresh iOS Debug simulator graph | Build succeeded across 89 targets | Current iPhone, Watch, widgets, Live Activities, Firebase integration, and app-target Swift compile together | Signed device, App Attest, BLE, or background execution |
...
| Repository tool suite | 36 passed | Tooling, legal, launch-gate, and workflow contracts remain intact | Native runtime behavior |
...
| Fictional pilot claim provisioning | First attempt failed safely at Google control HTTP 403; fixed missing OAuth quota-project binding, added a regression test, and the retry passed | Exactly the retained fictional test identity now carries the current pilot-admission attribute without printed credentials or identifiers | Deployed `pilot` runtime admission until the new image is applied |

## Physical device and deployment

- Install/update action: fresh unsigned iOS simulator graph built; Android
  packaging and simulator installation remain pending at this checkpoint.
- Generalized device and OS class: current iOS 26.5 simulator toolchain;
  Android emulator class pending.
- Data-preservation result: no physical-device data will be modified.
...
- Changed paths: managed server admission/configuration/tests; Apple and Android
  managed configuration, authentication, diagnostics, UI identifiers, and
  transport tests; guarded GCP variable; private pilot/relay tools and tests;
  ignored-file boundary; operations records.
- Commits: pending.
- Branch and remote state: `main` equals `origin/main` at the start.
...
- Decision-log entry: pending final deployment evidence.

## Open risks and honest limitations

- The loopback IAM relay is development-only. It proves native managed behavior
  without public ingress, but it is not a production mobile topology and must
  be stopped after the simulator run.
