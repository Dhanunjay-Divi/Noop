...
| Hosted Android execution-memory correction | PR `#16` candidate `ef9478ec` passed 32 hosted jobs, including Review Sample API 35, and completed 104/114 production-shell cases with zero failures before the bounded runner stopped the host at exact `resource-memory`/`125` evidence. Explicit APK preparation retains the project-wide 4 GiB heap, while the four emulator-test invocations use a 2 GiB Gradle heap and two workers under the unchanged 10% free-memory floor. Required-CI pins those boundaries. The retry classifier now requires positive bounded activity-service-loss evidence before its single infrastructure retry, so an unproven startup timeout fails closed. The exact command launches a real `-Xmx2048m` single-use daemon; 65 focused CI tests and the 305-test tool wall with one intentional skip pass, along with every repository policy gate. | The observed pressure is reduced at its owning test process without weakening the host guard, hiding a startup hang, or treating partial results as success | Replacement hosted completion of all 114 tests, OEM/physical behavior, or proof against every future hosted-runner image |
