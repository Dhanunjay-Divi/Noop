...
- State: `final exact-head review corrections implemented and all replacement local gates green; commit, protected integration, and physical-device evidence pending`
- Owner: project team
...
- The final Apple lifetime review closed two retained-screen edge cases.
  Workouts now retains the requested recovery key when tab departure cancels
  the current attempt, resumes that demand from the screen root even when the
  lazy sentinel remains off-screen at the retained scroll position, and uses a
  per-attempt token so an older canceled task cannot clear its replacement.
  Health now owns the explicit Live HR choice and foreground lease at retained
  screen lifetime; recycling the lazy heart-rate row resets only its display
  packet boundary, while actual screen departure releases the lease exactly
  once.
- The next exact-head review found five additional retry and task-ownership
  defects. Apple Workouts now records input changes while hidden without
  restarting its sequential HR reads until the retained tab is visible.
  Android Workouts identifies cleanup by a unique attempt number, so an older
  canceled A request cannot clear a replacement A request after an A-B-A input
  cycle. Android Today distinguishes a successful absent card value from a
  failed Room read and publishes its retained signature only after all five
  pinned-card reads succeed. Classic Today age-bounds its history-wide cache at
  two minutes, so a legacy facade's transient empty fallback is retried rather
  than restored for the process lifetime. Apple auto-workout CPU workers are
  linked to parent cancellation and check cancellation between dense phases,
  preventing a superseded scan from continuing into the step query or second
  classifier.
...
| Apple simulator build and UI performance | The final exact-source iOS build passed; repeated five-tab navigation passed in 80.831 s and Today scroll passed with a 5.202 s average measured window, 0.175 s CPU, and 69,271 KB peak app memory | The optimized simulator path is functional, nonblank, and emits no severe over-150 ms scroll hitch | Representative phone thermal, storage, BLE, or long-running performance |
...
| Exact-head automated and fresh review | The initial hosted passes corrected Android metric revision and retry gaps, Apple device and post-backfill ownership, re-paired Sleep motion, bounded Today caches, retained Workouts jobs, realtime-HR leases, strength-media cleanup, and cross-device auto-detection. The latest exact-head pass then found five remaining defects: hidden Apple Workouts restarts, Android A-B-A recovery cleanup, failed Android pinned-card reads cached as success, indefinitely retained Classic Today fallback empties, and unlinked Apple auto-workout CPU workers. All five are corrected; focused Android tests and 19 Apple retained-screen contracts compile and pass, followed by the complete replacement Android, macOS, iOS, and policy lanes | The replacement patch closes concrete stale-data, retry, lazy-lifetime, lease-ownership, cancellation, and cross-device publication defects before protected merge | Hosted exact-head review/checks, physical-device behavior, and the affected tester's reviewed report |
...
| Final Android local production lane | 4,176 JVM tests passed with zero failures or errors and 7 skips; APK assembly, lint, and instrumentation-source compilation passed in 4m25s after the five final review corrections | The exact Android source compiles, packages, preserves diagnostics privacy contracts, and passes its complete local unit/lint gate | OEM frame pacing, BLE, background survival, and physical battery impact |
...
| Final Apple local production lanes | After the five final review corrections, 19 retained-screen contracts passed; the macOS scheme passed 1,700 tests with zero failures and 1 expected skip; the iPhone 17 Pro simulator production shell passed 35 tests with zero failures and 1 expected skip in 586.7s. The final five-repeat Today scroll measured 5.185s average automation time, 0.172s average app CPU, and about 69.2 MB peak physical memory | The exact Apple source compiles, hidden tabs cannot restart recovery work, superseded auto-workout workers cancel before later phases, bounded cache retries remain mounted, the shared suite passes, and production-shell journeys execute on iOS 26.5 simulator | Physical iPhone frame pacing, BLE/background survival, energy, thermal behavior, or signing |
