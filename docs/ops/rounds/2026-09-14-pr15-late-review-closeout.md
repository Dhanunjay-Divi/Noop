...
- State: `supplier-independent product, simulator, localization, repository policy, static, operations, and mocked-infrastructure walls plus the bounded local final-review commit are complete; commit-bound evidence, hosted checks, protected integration, privacy restoration, and cleanup remain`
- Owner: project team
- Branch: `codex/product-safety-quality-audit-20260911`
- Start commit: `f7350d6eb9cee1dd9a903e5ab73d16f049607bc6`
- Current committed head before the final review corrections:
...
- A wall-clock rollback before the durable timezone-history tail now fails
  closed on Apple instead of extending the old zone's open-ended segment. The
  durable history remains unchanged and pending work is left for retry,
  matching Android's existing rejection contract.
- A changed historical day-owner row now invalidates the UTC civil day plus
  the full supported offset envelope on Apple and Android. The bounded range
  covers the intended local day for every offset from -18 to +18 hours, so the
  phone's current timezone cannot redirect the edit to an adjacent day.
...
- Body-composition target guidance now uses the actual profile weight
  confirmation state on Apple and Android. Restored or imported history may
  remain visible with provenance, but it cannot silently turn an unconfirmed
  profile default into weight-target guidance.
- The Apple Bluetooth scale card now shows BMI only when the reading is
  eligible to update the current profile. Unknown user slot `0xFF` and
  unassigned or differently assigned multi-user slots remain visible as
  measurements but cannot combine another person's BMI with the current
  profile's age and height.
...
|---|---|---|---|
| Apple focused, runtime, and regression evidence | All 75 hydration cases and the separate 181-case Safety, wind-down, notification-lifecycle, hydration-reminder, and accessibility matrix passed without failures or skips. The final complete macOS app suite executed 2,091 tests with 2,090 passes, one intentional Xiaomi-fixture skip, and zero failures. The bounded `NOOPiOS` Debug scheme ran on an iPhone 17 Pro simulator: 39 UI tests executed, 38 passed, one private-pilot case was intentionally skipped, and none failed. Focused rollback and weight-scale presentation tests also passed | The tested app launches, renders, navigates, scrolls, exercises report review, hydration/sleep controls, onboarding, trends, nutrition, settings, strength-body-map interactions, fail-closed timezone rollback, and scale-user BMI ownership; the complete macOS wall covers the broader Apple source | Physical notifications, BLE, background lifetime, battery, haptics, hardware accessibility behavior, clinical accuracy, or unrelated host-process memory |
| Complete Swift package evidence | WhoopStore executed 537 tests with zero failures. NoopRemoteSync executed 131 tests with zero failures. StrandAnalytics executed 1,487 tests with 1,480 passes, seven documented fixture skips, and zero failures | The exact package wall covers storage migration, hydration resolution and retry evidence, managed-document behavior, shared schema drift protection, Safety policy, remote sync, conservative ownership invalidation, and analytics contracts | Supplier firmware, skipped external fixtures, physical transport, or clinical accuracy |
| Complete Android variant matrices | Serialized Full and Demo runs each executed 4,820 tests with 4,813 passes, seven intentional skips, and zero failures or errors. Compile, APK assembly, lint, and instrumentation-source compilation passed for both variants. Both lint XML reports contained 77 hints and 1,171 warnings, with no Error or Fatal entry. One combined lint attempt exhausted a 4 GiB lint JVM; isolated 8 GiB bounded reruns passed without changing source | The exact Android wall covers product logic, Safety defaults, atomic initial-location requests, authorization and location validation, generated resources, report-request lifecycle, navigation contracts, schema declaration, confirmed-weight guidance, historical ownership invalidation, and both build graphs | OEM persistence, force-stop/background behavior, physical BLE, battery, haptics, hardware accessibility behavior, signed distribution, or future toolchain compatibility |
| Current Android API 35 shell | A clean disposable native-arm API 35 emulator ran 114 instrumentation tests: 112 passed, two intentional private-pilot cases were skipped, and none failed or errored. During compilation, one stale test identifier in the managed-document replay assertion was corrected from the removed point timestamp to the new bounded range endpoints. The managed emulator was deleted after evidence capture | Real API 35 SQLite, WorkManager, migration, lifecycle, report-review, app-shell, managed-document invalidation, APK installation, and the exact worktree's instrumentation contracts | Every OEM lifecycle, physical capture/share behavior, public feedback ingress, production operations, the private pilot, or representative-device frame pacing |
...
| Complete localization evidence | App-wide generation produced 813 shared strings and 45 Android-only resources across nine locales. Feedback generation covered 86 strings across nine locales. Two ordered generation passes produced byte-identical SHA manifests across 46 generated files. All 6,025 Apple catalog values were unchanged; generator-only key movement was mechanically reversed so the committed catalog order remains stable. The strict whole-tree audit, 49 top-level audit tests, and three feedback-localization tests passed | The exact localization wall covers deterministic generated Apple and Android catalogs, strict key/locale contracts, and feedback resources without unrelated catalog churn | Human linguistic review of every locale or physical large-text traversal |
...
| Local command and Xcode invocation corrections | One unwrapped `xcodebuild -list` attempted automatic package resolution and exhausted local free space; the generated 779 MiB DerivedData tree was removed, and all later Apple work used the pinned wrapper and existing package graph. A first final iOS compile later stopped only when the external volume filled; after removing completed generated caches, the same exact graph succeeded. On September 17, the first exact-current iOS shell stopped before build with exit 74 because the `org.swift.swiftpm` user cache was a broken symlink to a removed round-owned RAM volume. Only that broken link and the failed 24 kB DerivedData/result residue were removed; a normal mode-700 cache directory was restored and the same bounded command passed. The expected stale terminology failure after the final source edits was corrected by regenerating the inventory and repinning only its reviewed SHA-256 | Failed local setup attempts were bounded, understood, and cleaned without changing product behavior; the exact-current iOS runtime shell and repository trust root are green | Hosted runner capacity or unrelated user-process behavior |
...
| Current repository policy wall | The exact-current repository-tool suite executed 281 tests with zero failures. The regenerated terminology inventory records 17,703 classified occurrences across 1,564 path/category groups, the active-use allowlist is unchanged, and the reviewed inventory digest is repinned. Release-control, required-CI, trusted-release, calibration-parity, terminology, legal, distribution-provenance, private-data, health-claim, strict localization, shell, workflow, JSON, launch-gate, operations-record, mocked-infrastructure, and diff gates pass | The final generated tree and repository-owned policy implementations agree on the exact local source, and no release control was weakened to obtain the result | Hosted exact-SHA checks, protected integration, physical devices, or external launch gates |
| September 17 continuation review and cleanup | Final unresolved review threads produced four additional P2 corrections: Apple rejects wall-clock rollback instead of extending stale timezone provenance; both platforms invalidate a historical owner day across every supported offset; restored/imported weight history cannot unlock target guidance before profile confirmation; and an unassigned Apple scale user cannot expose BMI against the current profile. Focused tests and all complete product and repository walls pass with the updated counts above. A fresh independent read-only diff review reported no P0-P2 finding. No simulator, emulator, Gradle, Xcode, or disposable server process remains; the API 35 managed device, synthetic PostgreSQL databases, temporary server environment, and disposable OpenTofu provider directories were removed after evidence capture. Live memory remained above the bounded floor with about 92 GiB free disk; the September 9 application-memory alert is historical | The final review findings, exact local product and policy walls, both simulator runtimes, fresh review, and round-owned runtime cleanup are recorded | Hosted exact-SHA checks, physical devices, or external launch gates |
...
- Commits: committed baseline head `4afdb35f` is synchronized with the remote.
  The bounded local final-review commit is the commit containing this record;
  it is not yet pushed. Generated terminology and every local policy gate pass.
...
- Local repository-policy wall: all 281 repository-tool cases pass. The
  complete exact-current static, localization, privacy, claims, legal,
  calibration, terminology, required-CI, trusted-release, launch-gate,
  mocked-infrastructure, operations-record, and diff walls also pass.
- Preliminary immutable evidence for `6e09acd0`: all nine release controls
  ...
  two-artifact manifest verified. Commit-bound evidence for the commit
  containing this record is generated after commit creation; its private
  manifest is authoritative so this file does not attempt to embed its own Git
  SHA.
- Hosted exact-SHA checks: the current remote head is `4afdb35f`. Its results
  are historical because the final local corrections are uncommitted;
  replacement-head checks remain pending.
...
1. Generate and verify release evidence for the commit containing this record,
   then push once as one replacement head.
2. Wait for hosted exact-SHA checks, resolve the seven threads only against
   their proven replacement head, integrate through protection, restore
   repository privacy, verify protected `main`, synchronize the canonical
   checkout, and clean only round-owned resources.
