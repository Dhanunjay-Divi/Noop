...
- The newest exact-head review found two final state-lifetime defects. Android's
  cached-day collector previously waited for an older adaptive evaluation to
  finish before a new database emission could invalidate it. The collector now
  invalidates the active generation before forwarding each emission and uses
  latest-only collection, so the replacement cancels the older evaluation
  without allowing it to commit stale sleep or readiness guidance.
- Apple delivery-state migration already canonicalized the legacy four-field
  workout fingerprint, but the persisted contextual action could retain the
  legacy ID and be removed during the same reconciliation. The action center
  now discovers any equivalent legacy identity independently of the delivery
  state's current shape, then atomically migrates the active Workouts action,
  processing state, and bounded dismissed/completed tombstones to the canonical
  three-field ID before stale-action reconciliation. Existing copy, evidence,
  route, creation, and expiry remain unchanged, and a prior dismissal cannot
  reappear after migration.
- The latest exact-head review found two final launch and migration defects.
  Apple's repository publishers could queue contextual evaluation before the
  launch receipt and current Terms had opened the operational runtime. Every
  contextual scheduling and evaluation entry point now fails closed until that
  runtime opens, and the first evaluation is requested only after
  `operationalWorkStarted` becomes true. EventKit access and contextual
  notification work therefore cannot run behind the launch gate.
- Android canonicalized the delivery fingerprint but could leave the active
  Workouts action and its processing, dismissal, or completion identity under
  the legacy four-field ID. Recovery now migrates the active action and all
  bounded identity sets together before stale-action reconciliation. Copy,
  evidence, route, creation, expiry, and the user's prior decision remain
  unchanged under the canonical three-field ID.
- The latest exact-head review found two final priority and consent-boundary
  defects. Android's direct timezone-change path could post travel guidance
  while a stale planned-workout action, notification, and cooldown owner still
  existed; it now invalidates the in-flight evaluation and retracts those
  artifacts before travel enters the shared delivery policy. Apple Calendar
  opt-out or permission loss could leave an already-delivered workout prompt
  until queued reevaluation ran; every disable, denial, and unavailable path
  now clears the snapshot, cancels pending boundary work, and synchronously
  reconciles the delivered notification, Workouts action, expiry state, and
  cooldown owner before reevaluation is queued.
- The stale-head hosted Android production-shell run then exposed an independent
  report-liveness defect: all instrumentation cases executed, but the app-report
  review could omit `app-session-current.jsonl` when slow OS exit or ANR capture
  occupied the same serial executor as live breadcrumbs and the bounded report
  snapshot. Historical exit capture now has its own daemon queue, completed
  history is the only history eligible for a report, and the live-session queue
  retains a five-second bound. Queue rejection, timeout, interruption, or
...
- The next exact-head review found one final background-entry defect:
  `HealthConnectSyncWorker` could call the shared Android evaluator after a new
  Terms version invalidated the accepted receipt. `AdaptiveDayEvaluator` now
  requires `ManagedRuntimeGate` before creating an evaluation generation,
  observing timezone state, reading Room or Calendar, recording guidance
  diagnostics, or posting. The boundary therefore covers Health Connect, BLE,
  UI, and any future caller without relying on every caller to repeat the gate.
- The September 11 protected-review closeout found three remaining restart and
  provider-state gaps. Apple and Android now mark timezone state stale whenever
  the operational runtime is blocked, then consume that marker exactly once
  after current access and Terms reopen by replacing the baseline with the
  current offset and discarding any travel change observed while guidance was
  forbidden. Apple covers both its launch-access receipt and Terms gate.
- Android Calendar Provider refresh now distinguishes a confirmed empty result,
  provider failure, and a superseded evaluation. A null provider cursor is a
  failure rather than evidence that the user has no workout. Lifecycle and
  provider-edit invalidation makes the prior snapshot stale without deleting
  it, so a transient provider failure preserves the last confirmed plan and
  its delivery artifacts; explicit opt-out and permission loss still clear
  them immediately.
- Android planned-workout delivery now persists the exact generic workout
  fingerprint atomically with the shared prompt owner. If the process stops
  after notification delivery but before private adaptive-day state is saved,
  restart restores that state only when the owner's persisted fingerprint
  still identifies the current workout. A moved workout or legacy
  identity-less owner is reconciled as stale, and shared-slot cancellation
  remains inside the delivery-ledger lock so a newer adaptive-day prompt is
  preserved.
...
- Schema or migration impact: no app schema migration. Apple temporarily
  ...
  persisted.
- Existing-data retention impact: no health or history rows change. Legacy
  Apple and Android contextual action IDs and their bounded
  dismissal/completion tombstones migrate in place without recreating a
  dismissed action.
...
- Final state-lifetime correction coverage: no new diagnostic event is needed.
  Android reuses the existing fixed adaptive-day stale, cancelled, suppressed,
  and accepted outcomes; the generation invalidation itself has no payload.
  Apple changes only bounded local action state before the existing
  reconciliation lifecycle. Neither path records action fingerprints, calendar
  values, health values, user text, or arbitrary errors.
- Final launch-gate and Android migration coverage: no new diagnostic event is
  needed. Apple prevents calendar and contextual work before the operational
  runtime rather than emitting another state transition. Android migrates only
  bounded private action identity state before the existing accepted, stale,
  cancelled, and suppressed lifecycle. Neither path records action
  fingerprints, calendar values, health values, user text, or arbitrary
  errors.
- Final priority and opt-out correction coverage: no new diagnostic event is
  needed. Both corrections reuse the existing fixed stale, cancelled,
  suppressed, and accepted adaptive-day lifecycle. Cleanup occurs before the
  next delivery decision and records no calendar value, health value,
  fingerprint, user text, or arbitrary error.
- Final report-liveness correction coverage: historical capture records only a
  fixed `completed` or `failed` outcome. A degraded report contains one bounded
  `diagnostics.snapshot_unavailable` event with one of four allowlisted reasons:
...
- Final evaluator runtime-gate coverage: no new event is emitted when the
  current Terms receipt is absent. The evaluator returns before generation,
  timezone, repository, Calendar, diagnostics, or delivery work, so the
  unauthorized attempt itself retains no health, calendar, device, or error
  payload.
- Final operational-resume and orphan-recovery coverage adds two bounded fixed
  outcomes: `adaptive_day.time_zone_baseline` with
  `rebased_after_operational_block`, and
  `adaptive_day.planned_workout_state` with
  `restored_from_delivery_owner`. Neither event includes an offset, timestamp,
  fingerprint, calendar value, health value, user text, device identity, or
  arbitrary error. Calendar refresh continues to use only the existing fixed
  `matched`, `empty`, `failed`, `superseded`, permission, and access-change
  outcomes plus a zero/one/multiple candidate bucket.
...
| Final stale-evaluation and action-ID closeout | 40 focused Apple tests passed; both focused Android Calendar and adaptive-day suites passed | A new health-data emission invalidates and cancels the older Android evaluation before it can commit; Apple preserves the active Workouts action and prior dismissal across legacy-to-canonical fingerprint migration and process reload | Physical provider callbacks, process termination at every instruction, notification presentation, or background execution |
...
| Final evaluator runtime-gate closeout | The focused source-order contract passed in both Android variants; the complete Android wall passed 4,234 tests with seven expected skips plus both APK assemblies, full debug compile, lint, and instrumentation-source compilation | Every Android adaptive-day caller requires the current Terms receipt before generation, timezone, repository, or Calendar work | Physical worker startup, process termination at every instruction, or OEM scheduling |
| September 11 restart/provider closeout | 55 focused Apple tests passed; the focused Android Calendar and adaptive-day notifier suites passed; the complete Android wall passed 8,488 variant test executions with 14 expected skips, both APK assemblies, both instrumentation-source compilations, and both lint variants | Operational blocking rebases timezone state before guidance resumes, null/provider failures preserve confirmed Calendar state, superseded refreshes cannot publish, and process-death restoration requires the exact persisted generic workout identity | Physical permission/provider behavior, terminated-process notification presentation, OEM scheduling, signing, or store delivery |
| Complete macOS app suite | 1,750 passed, 1 expected external-data skip, 0 failures | The complete shared app integration, generated localization, exact-start identity, stale-artifact reconciliation, bounded retry policy, preserved natural-expiry cooldown, background-wake policy, consent reconciliation, synchronous opt-out cleanup, current check-in and sleep-target invalidation, launch-gated contextual work, latest-only Android parity contract, cross-platform action-ID migration, cancellation contract, generation supersession, travel-priority cleanup, and affected routing graph pass together | iOS runtime or physical Calendar behavior |
...
| Repository policy and terminology | 227 Tools tests passed after reviewed snapshot regeneration; release controls retained 9 checks, required CI retained 10 contexts, all 51 operations records validated, and terminology retained 17,371 occurrences across 1,512 classified path/category groups with reviewed active-allowlist SHA `77fd12aef2246bf73486eed92d4e8f8e9e9981caf3c2ac158b3d671448731bc7`, inventory SHA `2a86a3d84bb61d235da82a3424cd188c339f26537fa2c9ceb01d283c242bd13d`, and zero forbidden mappings. i18n, health-claims, calibration parity, legal provenance, and private-data gates also passed. | The final source and evidence preserve required CI, durable-record, privacy, and terminology controls; the active allowlist and category totals are unchanged, while reviewed source, test, and handoff locations moved with the correction | Independent legal review or physical-device behavior |
...
- Signed Android release probe: intentionally stopped by the repository's
  fail-closed private-signing guard before compilation because no private
  keystore was supplied. No signed artifact was created; the complete unsigned
  CI-equivalent Android wall passed.
...
- Physical-device handoff:
  `docs/handoff/CALENDAR-AWARE-PHYSICAL-DEVICE-RUNBOOK.md` records 22 exact
  synthetic-data scenarios, notification ownership checks, accessibility and
  performance conditions, bounded evidence rules, and the local UI-test runner
  limitation for the next agent.
...
- Branch and remote state: protected pull request `#14` remains open; the final
  source and evidence are locally verified and remain unpushed at the owner's
  request to avoid triggering hosted GitHub Actions. Normal merge is pending an
  explicitly authorized push, fresh exact-head review, replacement required
  checks, and protected merge without bypass.
...
1. Retain the complete local commit without pushing until the owner explicitly
   authorizes hosted checks, then obtain a fresh clean exact-head review and
...
2. Run the permission, provider-change, foreground/background notification,
   ...
   physical devices before release by following
   `docs/handoff/CALENDAR-AWARE-PHYSICAL-DEVICE-RUNBOOK.md`.
