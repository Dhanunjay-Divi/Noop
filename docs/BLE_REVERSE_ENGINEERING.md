...
| 63 | `motion_wear_quality` (u8) {0,1,2} | A 3-valued byte kept **raw**. Historical code also labelled these values still/walk/run, but that conflicts with the wear/contact interpretation and has no synchronized gait validation. It is not step or activity authority. |
