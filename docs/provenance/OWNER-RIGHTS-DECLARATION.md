...
container images, trademarks, firmware, or other material that NOOP does not
own. Independent third-party dependencies remain under their own terms, and
their required license and notice texts remain in `NOTICE` and
`ThirdPartyNotices/`.

This file records the repository owner's representation and release
authorization. It is an engineering provenance record, not an opinion from
