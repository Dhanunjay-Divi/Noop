...
| NOOP+ managed storage | **Implemented and locally verified; synthetic deployment under repair and physical evidence pending** | Phone OTP/App Check, per-installation credentials, explicit consent, immutable chunk upload, processor validation, snapshot/incremental restore, revocation, erasure, optional validated seven-day raw plus 30-day essential local retention, and complete managed-history ZIP export are implemented on Apple and Android. Firebase identity/App Check, Cloud SQL, API, processor, lifecycle, scheduler, KMS, Pub/Sub, and storage are deployed in synthetic Mumbai staging with an IAM-only API. The deployed lifecycle image has a PostgreSQL parameter-typing defect; corrected source and a real-PostgreSQL regression test exist, but replacement-image and synthetic end-to-end evidence are still required. No released mobile client is configured, the public invoker remains disabled, and no real health data is permitted in staging. |
| NOOP+ managed Friends | **Implemented and locally verified; private staging and physical delivery pending** | Apple and Android provide random rotatable exact-match IDs, profile and expiring invite links, mutual request acceptance, directional per-friend sharing of only six daily fields, removal/block/delete, non-competitive badges, and receiver-controlled bounded pokes. PostgreSQL 14 integration, the complete server suite, native client tests, Android compile/lint, and the full iOS simulator graph pass. Links currently use a custom app scheme, and pokes rely on foreground/background catch-up before generic local notification and eligible worn-band haptic; HTTPS universal/app links, APNs/FCM wake delivery, abuse/support operations, private-staging migration 025, and physical-device evidence remain open. |
