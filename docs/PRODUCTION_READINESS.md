...
| Managed app Safety paging | **Source and simulator verified; signed physical push pending** | Manual app-to-app pages create bounded 8- or 12-hour incidents. APNs/FCM payloads contain only a fixed event kind, opaque incident reference, and expiry; authenticated clients fetch details and may respond or decline. Push-registration rotation, late registration during an active page, bounded retry, owner cancel/resolve, expiry, latest-only location replacement, relaunch reconciliation, and privacy-safe diagnostics are covered. No physical terminated/background APNs or FCM delivery, user-permission journey, or OEM/iOS battery behavior is claimed. This is contact paging, not emergency dispatch. |
