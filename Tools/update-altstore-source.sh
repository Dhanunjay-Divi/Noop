...
# The downloadURL points at the canonical GitHub release asset
# (github.com/Dhanunjay-Divi/Noop/releases). Everything else reads CFBundleVersion + size from the IPA,
...
ORG="${NOOP_RELEASE_GITHUB_OWNER:-Dhanunjay-Divi}"
REPO="${NOOP_RELEASE_GITHUB_NAME:-Noop}"
...
# altstore-source.json lives at this repository's root. Keep an override for
# release automation that deliberately stages a separate manifest checkout.
...
URL="https://github.com/${ORG}/${REPO}/releases/download/v${VERSION}/NOOP-ios-unsigned-v${VERSION}.ipa"
