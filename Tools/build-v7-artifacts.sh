...
if [ -d "$IOSAPP" ]; then
  ...
  # has no runtime dependency on the watch bundle. Apply the replaceable capability-template signature
  # only after this final bundle mutation so the app/widget seals remain valid.
