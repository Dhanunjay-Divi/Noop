...
class RequiredCIGateTests(unittest.TestCase):
    ...
    def test_android_infrastructure_retry_is_bounded_and_fail_closed(self) -> None:
        ...
        self.assertEqual(source.count("timeout-minutes: 35"), 2)
        self.assertEqual(source.count("continue-on-error: true"), 1)
        self.assertIn("Tools/android-managed-device-retry.py", source)
        self.assertEqual(source.count("Tools/run-bounded-command.py"), 4)
        self.assertEqual(source.count("--timeout-seconds 420"), 2)
        self.assertEqual(source.count("--timeout-seconds 600"), 1)
        self.assertEqual(source.count("--timeout-seconds 180"), 1)
        self.assertEqual(source.count("--status-file "), 4)
        self.assertEqual(source.count("app/build/noop-managed-device-status/"), 7)
        self.assertEqual(source.count("--no-daemon"), 4)
        self.assertEqual(source.count("--no-configuration-cache"), 4)
